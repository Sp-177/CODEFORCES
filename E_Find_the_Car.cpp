...
void Solve(){
    ...
    while(q--){
        ...
        ll meter;cin>>meter;
            ...
            ll index=lower_bound(a.begin(),a.end(),meter) -a.begin();
            if(a[index]>meter){
                ll c_time=b[index]-b[index-1],extra=meter-a[index-1],c_dist=a[index]-a[index-1];
                ans=(c_time*extra/c_dist);
                ans+=b[index-1];
            }
            else if(a[index]==meter){
                ans=b[index];
            }
        cout<<ans<<' ';
    }
