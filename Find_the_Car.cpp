...
void Solve(){
    ...
    while(q--){
        ...
        if(meter==0)ans=0;
            ...
            vector<int>::iterator low1=lower_bound(all(a),meter);
            int index=low1-a.begin();
            if(a[index]>=meter){
                int travel=meter-(index-1>=0?a[index-1]:0);
                ld time=((b[index]-(index-1>=0?b[index-1]:0))/(1.0*(a[index]-(index-1>=0?a[index-1]:0))));
                ans=travel*time+(index-1>=0?b[index-1]:0);
            }
            else{
                int travel=meter-a[index];
                ld time=((b[index+1]-b[index])/((a[index+1]-a[index])*1.0));
                ans=travel*time+b[index];
            }
        }
        cout<<ans<<' ';
    }
