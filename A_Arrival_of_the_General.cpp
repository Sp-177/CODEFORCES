...
void Solve()
{
    ...
    int index_high=0,index_low=0;
    bool flg=true;
    F(0,n,1,i){
        if(v[i]==maxi&&flg){index_high=i;flg=false;}
        if(v[i]==mini)index_low=i;
    }
    // cout<<index_high<<' '<<index_low<<endl;
    if(index_high>index_low)cout<<abs(index_high-0)+abs(n-1-index_low)-1<<endl;
    else if(index_high<index_low)cout<<abs(index_high-0)+abs(n-1-index_low)<<endl;
    else cout<<0<<endl;
