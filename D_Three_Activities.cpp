...
void Solve()
{
    int n;cin>>n;pq<vi> pq1,pq2,pq3;
    F(0,n,1,i){int x;cin>>x;pq1.pu({x,i});}
    F(0,n,1,i){int x;cin>>x;pq2.pu({x,i});}
    F(0,n,1,i){int x;cin>>x;pq3.pu({x,i});}
    ll sum=0;
    // cout<<sum<<endl;
	cnt=3;
	si s;
	while(pq1.sz&&pq2.sz&&pq3.sz&&cnt){
		int d1=pq1.top()[1],d2=pq2.top()[1],d3=pq3.top()[1];
		if(d1!=d2&&d2!=d3&&d3!=d1){cnt=0;sum=pq1.top()[0]+pq2.top()[0]+pq3.top()[0];}
		else {
			if(d1==d2&&d1!=d3){
