...
void Solve()
{
     ...
     int n,m;cin>>n>>m;
     vi pro_arr(n+m+1),test_arr(n+m+1);inp(pro_arr);inp(test_arr);
     vi nature(n+m+1),test_as_pro,pro_as_test,job;
     int total=n+m+1;
     ll sum=0;
     F(0,total-1,1,i){
            if(pro_arr[i]>test_arr[i]){
                nature[i]=1;
                if(n){
                sum+=pro_arr[i];
     ...
     nature[total-1]=pro_arr[total-1]>test_arr[total-1]?1:0;
    job.pb(1);
     vl ans;
     ans.pb(sum);
    // pr(nature);
    // pr(job);
     ...
     Fr(total-2,-1,1,i){
        ...
        if(j==1){
            ele-=pro_arr[i];
            if(pro_as_test.sz){
                int last=pro_as_test.back();
                ele+=pro_arr[last];
                ele-=test_arr[last];
                ele+=test_arr[total-1];
            }
            else{
                ele+=pro_arr[total-1];
            }
            if(nature[i]==0){test_as_pro.pb(i);}
