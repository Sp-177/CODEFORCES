...
while(end<n){
    st.insert({arr[end],end});
    while(st.size()&&(st.rbegin()->first-st.begin()->first)>k){
        st.erase({arr[start],start});
        start++;
    }
    if(st.size()&&(st.rbegin()->first-st.begin()->first)<=k)ans+=end-start+1;
    end++;
}
cout<<ans<<endl;
